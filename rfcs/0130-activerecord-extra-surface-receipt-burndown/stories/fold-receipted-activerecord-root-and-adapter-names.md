...
Split from `converge-receipted-activerecord-root-and-adapter-names`, which converged `ExplainRegistry.collectEnabled` (→ `collectQ`, explain_registry.rb:13,29), `isSuppressed` (→ `Suppressor.registry`, suppressor.rb:51), `PoolConfig#connectionSpecName` (→ `db_config.name`, pool_config.rb:8), `ConnectionHandler#clearAllConnections` (→ `clear_all_connections!`, connection_handler.rb:178), `TransactionState#rolledBack` (→ `rolledback?`), `AbstractReflection#isThrough` (→ `through_reflection?`) and `escapeComment` (folded into `escape_sql_comment`, query_logs.rb:213). The rest remain here. Note: `compareTo` is Ruby `<=>` (adapter_specific_registry.rb:63) and `toParamClass` collides with the instance `toParam` export in integration.ts (integration.rb:57,147).
...
`@noRailsEquivalent CONVERGEABLE fold-receipted-activerecord-root-and-adapter-names` instead: each is live trails surface with
...
| `packages/activerecord/src/connection-adapters/abstract/transaction.ts`          | `TransactionCallback`                                                                                                                                                             |
...
| `packages/activerecord/src/connection-adapters/pool-config.ts`                   | `poolInitialized`                                                                                                                                                                 |
...
| `packages/activerecord/src/explain-registry.ts`                                  | `collectingQueries`                                                                                                                                                               |
...
| `packages/activerecord/src/query-logs.ts`                                        | `clearContext`                                                                                                                                                                    |
| `packages/activerecord/src/querying.ts`                                          | `withCte`                                                                                                                                                                         |
| `packages/activerecord/src/reflection.ts`                                        | `computeForeignKey`                                                                                                                                                               |
...
- `git grep fold-receipted-activerecord-root-and-adapter-names` returns nothing when the story closes.
