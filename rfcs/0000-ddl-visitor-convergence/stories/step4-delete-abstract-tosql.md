...
- [ ] The blank-type guard ("Column X has an empty or blank type") currently at
      ~L1161–1165 in `toSql()` is moved to `SchemaCreation.typeToSql` (`abstract/schema-creation.ts`):
      in the `default` branch, before returning `String(type).toUpperCase()`, throw
      when `!type || !String(type).trim()`.
...
- [ ] All of the following test files pass: - `abstract/schema-definitions.test.ts` - `abstract/schema-creation.test.ts` (blank-type guard test added here) - `sqlite3/schema-definitions.test.ts` - `postgresql/schema-definitions.test.ts` - `mysql/schema-creation.test.ts`
...
once `toSql()` is removed from `AbstractTableDefinition`, the `typeof (node as any).toSql
=== "function"` check at `database-statements.ts:174` returns `false`, and it proceeds
to `toSqlAndBinds` which produces an error ("cannot convert TableDefinition to SQL").

The correct fix: `MigrationContext.createTable` should use
`this.connection.schemaCreation ? this.connection.schemaCreation.accept(td) : td.toSql()`
transitionally, or simply be updated to call the visitor directly. After Step 4,
`MigrationContext.createTable:1935` must change from:
...
```typescript
const ss = this.connection.schemaStatements
  ? this.connection.schemaStatements()
  : new SchemaStatements(this.connection);
await this.connection.executeMutation(ss.schemaCreation.accept(td));
```

or equivalent. This is the final cleanup that makes `MigrationContext.createTable`
structurally identical to `SchemaStatements.createTable` for DDL generation.
