...
`self.quoted_time` — which SQLite overrides to keep a leading `2000-01-01` date prefix
(`'2000-01-01 HH:MM:SS'`, vs the abstract `quoted_time`'s bare `HH:MM:SS`).
...
reviewing PR #3216-era work — see PR #3222 (quote/quote_table_name dispatch),
which fixed the abstract layer + PostgreSQL but deliberately left SQLite as a
separate, larger refactor.

## Acceptance criteria

- `sqliteQuote` follows Rails' structure: special-case `Numeric`
  (finite → delegate; non-finite → `'<value>'`) and route the remaining cases
  through the abstract `quote` via `abstractQuote.call(this, value)` instead of
  re-listing every branch inline.
- SQLite's `quotedDate` and `quotedTime` are exposed on the `SQLite3Adapter`
  class so the inherited dispatch lands on them (`quotedTime` keeps the
  leading `2000-01-01` date prefix; `quotedDate` matches the abstract).
- Output is unchanged for every existing case (string/binary/bool/symbol/date/
  time literals) — verified against current `sqlite3/quoting.test.ts`.
