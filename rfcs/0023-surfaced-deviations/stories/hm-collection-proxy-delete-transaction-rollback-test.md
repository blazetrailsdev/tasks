...
- Test names must match a Rails test verbatim (`parity:test` matches by name) and there is no Rails test covering the `delete`/nullify + transaction case (`test_transaction_when_deleting_persisted`/`_new_record` in has_many_associations_test.rb cover `destroy`, a different path).
