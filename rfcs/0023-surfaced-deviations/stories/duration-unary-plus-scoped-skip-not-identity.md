...
global `+@`→`identity` parity:api mapping. On review this was judged the wrong
shape:
...
- `identity()` is **inert dead code**. Ruby `+@` is the unary-plus operator;
  ...
  exists only to clear the parity:api missing-list.
...
a dead method, no parity:api regression.
...
`test_unary_plus` test, run `pnpm parity:api:conventions`.
...
- `parity:api --package activesupport` shows `duration.rb` with `+@` neither
