---
title: "Port ActiveSupport::Duration#+@ unary-plus + parity:api mapping"
status: done
updated: 2026-06-24
...
operator `+@` surfaced as an unported parity:api miss on
...
parity:api mapping was added; `+@` has no analog yet, so `duration.rb` still
...
decide the convention) and add the matching parity:api name mapping in
`scripts/api-compare/conventions.ts` (parallel to the `-@`→`negate` row), so
`parity:api --package activesupport` credits it instead of hunting an
impossible `+@` symbol.
...
- `duration.rb` no longer lists `+@` as missing in parity:api.
