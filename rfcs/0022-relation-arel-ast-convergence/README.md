...
| ID                                                                        | Title                                                | Cluster | Priority | Est LOC | Deps                                                                                   |
| ------------------------------------------------------------------------- | ---------------------------------------------------- | ------- | -------- | ------- | -------------------------------------------------------------------------------------- |
| [cte-build-with-expression-ast](stories/cte-build-with-expression-ast.md) | CTE array values → `Arel::Nodes::UnionAll` AST       | cte     | 1        | ~450    | —                                                                                      |
| [set-operations-arel-nodes](stories/set-operations-arel-nodes.md)         | Compose set-ops as Arel `Union*` nodes               | set-ops | 1        | ~400    | —                                                                                      |
| [set-operations-bind-threading](stories/set-operations-bind-threading.md) | Drop `$N` regex renumber; thread binds via collector | set-ops | 2        | ~300    | set-operations-arel-nodes                                                              |
| [from-clause-arel-manager](stories/from-clause-arel-manager.md)           | `from()` on the manager pre-compile (`build_from`)   | from    | 1        | ~400    | —                                                                                      |
| [pluck-from-cte-threading](stories/pluck-from-cte-threading.md)           | `pluck` threads `from()` / CTE through the manager   | from    | 2        | ~300    | from-clause-arel-manager                                                               |
| [arel-ast-convergence-verify](stories/arel-ast-convergence-verify.md)     | Verify `test:compare` / `api:compare` deltas ≥ 0     | verify  | 3        | ~120    | cte-build-with-expression-ast, set-operations-bind-threading, pluck-from-cte-threading |
