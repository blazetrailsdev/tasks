...
cluster: null
packages: []
deps: ["port-test-date-strptime-frags"]
deps-rfc: []
est-loc: 240
priority: null
pr: 6332
...
This story ports **Lines 306-end: the public `Date.strptime` / `DateTime.strptime`, the two-digit-year completion arm, the raise arms and the `%s`/`%z` cases. Starts at `test_strptime` (`:306`).**

Ruby source: `vendor/date/test/date/test_date_strptime.rb`
Target: `packages/date/src/test-date-strptime.test.ts` (the convention name `test:compare` expects;
...
- `test_strptime`
- `test_strptime__2`
- `test_strptime__minus`
- `test_strptime__comp`
- `test_strptime__d_to_s`
- `test_strptime__ex`
- `test_given_string`
- `test_sz`
...
- [ ] Every listed test is ported into `packages/date/src/test-date-strptime.test.ts` under its Ruby
...
- [ ] Lands **after** [[port-test-date-strptime-frags]], which creates
      `packages/date/src/test-date-strptime.test.ts`. Both stories write the same file, so they
