...
This story ports **The whole file: a single test, ~30 lines. Smallest story in the set — good first one to land, because it proves the enrollment wiring end to end at near-zero cost.**

Ruby source: `vendor/date/test/date/test_date_compat.rb`
Target: `packages/date/src/test-date-compat.test.ts` (the convention name `test:compare` expects;
...
- [ ] Every listed test is ported into `packages/date/src/test-date-compat.test.ts` under its Ruby
