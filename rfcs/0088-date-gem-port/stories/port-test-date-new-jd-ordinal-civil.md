...
This story ports **Lines 7-214: the `Date.jd`, `Date.ordinal` and `Date.civil` constructors with their negative-index, raise and calendar-reform arms, plus the shared invalid-type guards. Stops at `test_commercial` (`:215`).**

Ruby source: `vendor/date/test/date/test_date_new.rb`
Target: `packages/date/src/test-date-new.test.ts` (the convention name `test:compare` expects;
