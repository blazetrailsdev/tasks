...
This story ports **Lines 10-152: `#new_offset`, `#+`, `#-`, `#<=>` (including the `__ex` raise arms) and the whole `prev_*` family. Stops at `test_next` (`:153`).**

Ruby source: `vendor/date/test/date/test_date_arith.rb`
Target: `packages/date/src/test-date-arith.test.ts` (the convention name `test:compare` expects;
