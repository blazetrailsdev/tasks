---
title: "assertions-activesupport-cache-xml-json-callbacks"
status: ready
updated: 2026-09-20
...
`callbacks_test.rb`, the `cache/` tail, `xml_mini` and `json`.

| Rails file                                | count | kind | value |  total |
| ----------------------------------------- | ----: | ---: | ----: | -----: |
| `callbacks_test.rb`                       |    16 |   26 |     1 |     43 |
| `cache/cache_store_setting_test.rb`       |     6 |    6 |     1 |     13 |
| `xml_mini_test.rb`                        |     5 |    5 |     0 |     10 |
| `json/encoding_test.rb`                   |     1 |    2 |     0 |      3 |
| `cache/stores/null_store_test.rb`         |     1 |    2 |     0 |      3 |
| `json/decoding_test.rb`                   |     1 |    1 |     0 |      2 |
| `ordered_options_test.rb`                 |     0 |    2 |     0 |      2 |
| `cache/behaviors/cache_store_behavior.rb` |     0 |    2 |     0 |      2 |
| `xml_mini/xml_mini_engine_test.rb`        |     0 |    1 |     0 |      1 |
| **total**                                 |    30 |   47 |     2 | **79** |
...
79 mismatches over 9 files — one PR. Note `callbacks_test.rb` here is
activesupport's, not activerecord's `associations/callbacks_test.rb` (owned by
`assertions-tail-associations-1b`); it carries 81 TS-only extras, by far the
largest extras count in the package, so expect a substantial `.trails.test.ts`
split. `cache/behaviors/cache_store_behavior.rb` is a shared behaviour module,
not a test file, and has 40 unported names — converge its 2 kind rows only.
