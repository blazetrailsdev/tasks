---
title: "assertions-activemodel-errors-cluster"
status: ready
updated: 2026-09-19
...
Errors / Error assertion burndown — activemodel's single largest cluster.

| Rails file       | count | kind | value |   total |
| ---------------- | ----: | ---: | ----: | ------: |
| `errors_test.rb` |    37 |   65 |     0 |     102 |
| `error_test.rb`  |     8 |   16 |     3 |      27 |
| **total**        |    45 |   81 |     3 | **129** |
...
`errors_test.rb` is 86 tests / 102 mismatches and `error_test.rb` 28 tests / 27,
so the pair is inside the ~250-mismatch threshold and is meant to land as one PR.
`error_test.rb` also reports 12 extra (TS-only) tests — move genuine trails-only
assertions to a `.trails.test.ts` sibling rather than leaving them inflating a
mirrored test's count.
