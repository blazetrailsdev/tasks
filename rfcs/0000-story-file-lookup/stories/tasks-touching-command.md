...
(`scripts/cli.ts:3420-3716` has no such case; `usage()` at `:3721` has no such
...
`TASKS_DIR`.** The per-worktree `tasks` symlink (`scripts/cli.ts:78-89`, arm 3)
...
- [ ] `storiesTouching(index, query, opts)` exported from `scripts/cli.ts` near
      `listFiltered()` (`:601`), matching in order: exact path, directory
...
- [ ] Open statuses (`draft`, `ready`, `claimed`, `in-progress`, `blocked`) are
      ...
      Drafts must be included — 776 of 1,164 open stories are drafts, and a
...
- [ ] Dispatch case added using `readIndexSource().index`; `"repo"` added to the
      `valueFlags` array (`:3425-3445`); a `touching` entry added to `usage()`.
- [ ] `--json` emits `{ query, path_churn_90d, churn_verdict, trails_repo,
  stories }`; human output reuses `formatRows()` (`:3280`) under a churn
      banner.
...
# expect ~50 open stories across 6 RFCs, churn verdict: hot
