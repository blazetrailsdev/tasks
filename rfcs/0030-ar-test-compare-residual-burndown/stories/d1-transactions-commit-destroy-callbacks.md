...
Part of RFC 0030-ar-test-compare-residual-burndown (parity:test residual burndown). Reopens f4. committedBang fires for destroy (triggerDestroyCallback), callback ordering, and instrumentTransaction Notifications event not published.

**41** `it.skip` tests to un-skip across 3 file(s) (deduped; permanent-skips — Marshal/YAML/thread/fork/Rational — excluded). For reference, `parity:test` reports **34** `matchedSkipped` for these files (snapshot 2026-06-15); any delta is permanent/​gated skips not on the un-skip list.
