...
Enumerated hit(s) in `packages/activerecord/src/associations/has-and-belongs-to-many-associations.test.ts`:

- lines 775, 797: `projectIds ... .sort((a, b) => a - b)`
...
- [ ] Converge each id-arithmetic / `record.id` assertion in `packages/activerecord/src/associations/has-and-belongs-to-many-associations.test.ts` to be green
