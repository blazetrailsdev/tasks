...
Enumerated hit(s) in `packages/activerecord/src/relations.test.ts`:

- line 5172: `[...ids].sort((a, b) => a - b)`
...
- [ ] Converge each id-arithmetic / `record.id` assertion in `packages/activerecord/src/relations.test.ts` to be green
