...
priority: null
pr: 3413
claim: "2026-06-15T22:34:28Z"
assignee: "b1-relation-scoping"
blocked-by: null
---

## Context

Part of RFC 0030-ar-test-compare-residual-burndown (parity:test residual burndown). `relation/scoping.ts#scopeFor` / `Relation#scoped` missing Rails parity (scoping stack, merge).

**20** `it.skip` tests to un-skip across 1 file(s) (deduped; permanent-skips — Marshal/YAML/thread/fork/Rational — excluded). For reference, `parity:test` reports **20** `matchedSkipped` for these files (snapshot 2026-06-15); any delta is permanent/​gated skips not on the un-skip list.
