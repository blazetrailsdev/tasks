...
Part of RFC 0030-ar-test-compare-residual-burndown (parity:test residual burndown). preventingWrites enforcement, schema-cache hot reload, lazy-connection query cache, timestamp parity, db tasks tail.

**17** `it.skip` tests to un-skip across 8 file(s) (deduped; permanent-skips — Marshal/YAML/thread/fork/Rational — excluded). For reference, `parity:test` reports **13** `matchedSkipped` for these files (snapshot 2026-06-15); any delta is permanent/​gated skips not on the un-skip list.
