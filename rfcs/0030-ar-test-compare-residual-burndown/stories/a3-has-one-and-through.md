...
Part of RFC 0030-ar-test-compare-residual-burndown (parity:test residual burndown). has-one / has-one-through loader gaps (disable_joins variant + base through).

**29** `it.skip` tests to un-skip across 3 file(s) (deduped; permanent-skips — Marshal/YAML/thread/fork/Rational — excluded). For reference, `parity:test` reports **29** `matchedSkipped` for these files (snapshot 2026-06-15); any delta is permanent/​gated skips not on the un-skip list.
