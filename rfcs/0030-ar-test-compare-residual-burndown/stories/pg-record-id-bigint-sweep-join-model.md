...
Enumerated hit(s) in `packages/activerecord/src/associations/join-model.test.ts`:

- lines 797, 807, 819, 828, 843, 894, 1036, 1043: `.sort((a, b) => a - b)` over ids
...
- [ ] Converge each id-arithmetic / `record.id` assertion in `packages/activerecord/src/associations/join-model.test.ts` to be green
