...
Counted `test:compare` skips covered by this story: **6** (snapshot 2026-06-15, `pnpm test:compare --cached --json --package activerecord`).
...
- `adapter_test.rb` → `adapter.test.ts` — **3** counted skips:
...
- `adapters/sqlite3/explain_test.rb` → `adapters/sqlite3/explain.test.ts` — **1** counted skips:
  - explain with eager loading
- `adapters/postgresql/explain_test.rb` → `adapters/postgresql/explain.test.ts` — **1** counted skips:
- `adapters/abstract_mysql_adapter/mysql_explain_test.rb` → `adapters/abstract-mysql-adapter/mysql-explain.test.ts` — **1** counted skips:
