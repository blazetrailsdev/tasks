...
Part of RFC 0030-ar-test-compare-residual-burndown (parity:test residual burndown). Notifications.\_notify swallows subscriber errors + EXPLAIN parity across adapters.

**6** `it.skip` tests to un-skip across 4 file(s) (deduped; permanent-skips — Marshal/YAML/thread/fork/Rational — excluded). For reference, `parity:test` reports **6** `matchedSkipped` for these files (snapshot 2026-06-15); any delta is permanent/​gated skips not on the un-skip list.
