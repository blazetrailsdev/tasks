...
Part of RFC 0030-ar-test-compare-residual-burndown (parity:test residual burndown). InverseOfAssociationRecursiveError class + bidirectional destroy + collection/singular tail in associations_test.

**18** `it.skip` tests to un-skip across 5 file(s) (deduped; permanent-skips — Marshal/YAML/thread/fork/Rational — excluded). For reference, `parity:test` reports **18** `matchedSkipped` for these files (snapshot 2026-06-15); any delta is permanent/​gated skips not on the un-skip list.
