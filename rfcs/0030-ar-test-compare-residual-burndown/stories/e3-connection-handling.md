...
Part of RFC 0030-ar-test-compare-residual-burndown (parity:test residual burndown). StandaloneConnection parity + multi-db role connection handling + URL-config merge.

**6** `it.skip` tests to un-skip across 6 file(s) (deduped; permanent-skips — Marshal/YAML/thread/fork/Rational — excluded). For reference, `parity:test` reports **9** `matchedSkipped` for these files (snapshot 2026-06-15); any delta is permanent/​gated skips not on the un-skip list.
