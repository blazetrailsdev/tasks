...
Part of RFC 0030-ar-test-compare-residual-burndown (parity:test residual burndown). default_scope evaluation/inheritance/unscoped parity (untagged — needs root-cause triage first).

**16** `it.skip` tests to un-skip across 1 file(s) (deduped; permanent-skips — Marshal/YAML/thread/fork/Rational — excluded). For reference, `parity:test` reports **16** `matchedSkipped` for these files (snapshot 2026-06-15); any delta is permanent/​gated skips not on the un-skip list.
