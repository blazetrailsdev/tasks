...
Part of RFC 0030-ar-test-compare-residual-burndown (parity:test residual burndown). Reopens f6. Composite-primary-key support in the test adapter + Phase-G before_add-at-assignment callback timing.

**15** `it.skip` tests to un-skip across 2 file(s) (deduped; permanent-skips — Marshal/YAML/thread/fork/Rational — excluded). For reference, `parity:test` reports **22** `matchedSkipped` for these files (snapshot 2026-06-15); any delta is permanent/​gated skips not on the un-skip list.
...
- [x] `pnpm parity:test --package activerecord` shows these files with no `it.skip`-based `matchedSkipped` (any residual reclassified to a permanent-skip with a recorded reason per the RFC Deferred table).
