---
title: "pg-record-id-bigint-sweep-relation-with"
status: done
updated: 2026-06-21
...
Enumerated hit(s) in `packages/activerecord/src/relation/with.test.ts`:

- lines 22, 25, 165, 176, 186: `.sort((a, b) => a - b)` over ids
...
- [ ] Converge each id-arithmetic / `record.id` assertion in `packages/activerecord/src/relation/with.test.ts` to be green
