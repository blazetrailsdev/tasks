...
Part of RFC 0030-ar-test-compare-residual-burndown (parity:test residual burndown). `associations/join-model.ts` / `preloader.ts` miss join-model (HMT) semantics.

**35** `it.skip` tests to un-skip across 1 file(s) (deduped; permanent-skips — Marshal/YAML/thread/fork/Rational — excluded). For reference, `parity:test` reports **35** `matchedSkipped` for these files (snapshot 2026-06-15); any delta is permanent/​gated skips not on the un-skip list.
