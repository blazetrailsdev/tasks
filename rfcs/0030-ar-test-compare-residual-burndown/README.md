...
| In-place mutation (immutable JS strings) | `dirty_test.rb` (4): `attribute will change!`, `in place mutation detection`, `in place mutation for binary`, `mutating and then assigning doesn't remove the change` | permanent `it.skip` — JS strings are immutable; Rails' `catchphrase << " matey!"` has no faithful JS equivalent. The force-dirty (`attribute_will_change!`) API itself is implemented and exercised by `virtual attribute will change` + `attribute_will_change! doesn't try to save non-persistable attributes`. |
| Ruby singleton / reader-only override    | `dirty_test.rb` (3): `changes is correct for subclass`, `changes is correct if override attribute reader`, `getters with side effects are allowed`                    | permanent `it.skip` — Rails overrides only the generated reader (`def catchphrase; super.upcase; end`) or defines a per-instance singleton method; a JS subclass getter shadows the inherited getter/setter pair and a faithful reader-only-with-working-super-setter form has no JS equivalent.                  |
| Ruby symbol type-cast                    | `dirty_test.rb` (1): `string attribute should compare with typecast symbol after update`                                                                              | permanent `it.skip` — relies on a Ruby symbol auto-coercing to a string; JS has no equivalent and substituting `"foo"` would test the cast vacuously.                                                                                                                                                             |
| Bespoke per-test DDL (design)            | `dirty_test.rb` (1): `field named field`                                                                                                                              | stays skipped by design — would reintroduce per-test `create_table` (RFC 0028 DDL cost) for a column-name edge case covered by reflection tests elsewhere.                                                                                                                                                        |
...
| [current-savepoint-name-accessor](stories/current-savepoint-name-accessor.md)                                   | expose current_savepoint_name / Transaction#savepoint_name                                                | claimed     | 50      | —                |
| [d2-insert-all-returning-result](stories/d2-insert-all-returning-result.md)                                     | insert_all/upsert_all returning → ActiveRecord::Result                                                    | claimed     | 200     | persistence      |
| [d2-insert-all-unique-index-introspection](stories/d2-insert-all-unique-index-introspection.md)                 | insert_all unique_by schema-cache index introspection                                                     | claimed     | 250     | persistence      |
| [d3-nested-attributes-cpk-callbacks](stories/d3-nested-attributes-cpk-callbacks.md)                             | D3 — nested-attributes: CPK + before_add callback timing                                                  | claimed     | 120     | persistence      |
| [d4-dirty-force-dirty](stories/d4-dirty-force-dirty.md)                                                         | D4 — dirty: attribute_will_change! force-dirty path                                                       | claimed     | 80      | core-residuals   |
