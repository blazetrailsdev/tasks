...
- `packages/activerecord/src/invertible-migration.test.ts` → mirror `vendor/rails/activerecord/test/cases/invertible_migration_test.rb` (confirm it exists; if no 1:1 Rails file, the trails file is bespoke — delete it and port the Rails test cases that cover this behavior)
- `packages/activerecord/src/migration.test.ts` → mirror `vendor/rails/activerecord/test/cases/migration_test.rb` (confirm it exists; if no 1:1 Rails file, the trails file is bespoke — delete it and port the Rails test cases that cover this behavior)
- `packages/activerecord/src/migration.trails.test.ts` → trails-only extension — keep as-is unless it duplicates a Rails case; do NOT invent a Rails source
