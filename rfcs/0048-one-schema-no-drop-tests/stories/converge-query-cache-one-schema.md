---
title: "Converge query-cache tests to canonical (one-schema)"
status: in-progress
updated: 2026-06-30
...
- `packages/activerecord/src/query-cache.test.ts` → mirror `vendor/rails/activerecord/test/cases/query_cache_test.rb` (confirm it exists; if no 1:1 Rails file, the trails file is bespoke — delete it and port the Rails test cases that cover this behavior)
- `packages/activerecord/src/query-cache-mixin.test.ts` → mirror `vendor/rails/activerecord/test/cases/query_cache_mixin_test.rb` (confirm it exists; if no 1:1 Rails file, the trails file is bespoke — delete it and port the Rails test cases that cover this behavior)
