...
- `packages/activerecord/src/finder.test.ts` → mirror `vendor/rails/activerecord/test/cases/finder_test.rb` (confirm it exists; if no 1:1 Rails file, the trails file is bespoke — delete it and port the Rails test cases that cover this behavior)
- `packages/activerecord/src/enum.test.ts` → mirror `vendor/rails/activerecord/test/cases/enum_test.rb` (confirm it exists; if no 1:1 Rails file, the trails file is bespoke — delete it and port the Rails test cases that cover this behavior)
