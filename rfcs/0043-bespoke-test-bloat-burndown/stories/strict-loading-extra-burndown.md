...
`packages/activerecord/src/strict-loading.test.ts` carries **54 extra (TS-only) tests** -- `it`/`test`
declarations matching no Rails test (`test:compare` snapshot 2026-06-23,
matched=54, rubyTestCount=54). Per RFC 0043 these are
...
- **One file per PR:** touch only `packages/activerecord/src/strict-loading.test.ts` (optionally its sibling
...
- No test renamed or reworded; the remaining file passes
  (`pnpm vitest run packages/activerecord/src/strict-loading.test.ts`).
