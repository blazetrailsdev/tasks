...
`packages/activerecord/src/transactions.test.ts` carries **74 extra (TS-only) tests** -- `it`/`test`
declarations matching no Rails test (`test:compare` snapshot 2026-06-23,
matched=91, rubyTestCount=91). Per RFC 0043 these are
...
- **One file per PR:** touch only `packages/activerecord/src/transactions.test.ts` (optionally its sibling
...
- No test renamed or reworded; the remaining file passes
  (`pnpm vitest run packages/activerecord/src/transactions.test.ts`).
