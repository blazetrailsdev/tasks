...
`packages/activerecord/src/attribute-methods.test.ts` carries **56 extra (TS-only) tests** -- `it`/`test`
declarations matching no Rails test (`test:compare` snapshot 2026-06-23,
matched=133, rubyTestCount=133). Per RFC 0043 these are
...
- **One file per PR:** touch only `packages/activerecord/src/attribute-methods.test.ts` (optionally its sibling
