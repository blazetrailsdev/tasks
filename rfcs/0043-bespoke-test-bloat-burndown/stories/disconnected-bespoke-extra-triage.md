---
title: "Burn down 1 extra (TS-only) tests in disconnected.test.ts"
status: ready
updated: 2026-06-25
...
`packages/activerecord/src/disconnected.test.ts` carries **1 extra (TS-only) tests** -- `it`/`test`
...
`vendor/rails/activerecord/test/cases/disconnected_test.rb`.

This file's paired Rails test (`disconnected_test.rb`) maps **zero** tests in `test:compare` (its `it`s are all TS-only). Such connection/adapter infra tests are prime **RELOCATE** candidates (trails-specific invariants -> `*.trails.test.ts`) rather than DELETE -- triage each against the Rails file before removing.
...
- **One file per PR:** touch only `packages/activerecord/src/disconnected.test.ts` (optionally its sibling
...
- No test renamed or reworded; the remaining file passes
  (`pnpm vitest run packages/activerecord/src/disconnected.test.ts`).
