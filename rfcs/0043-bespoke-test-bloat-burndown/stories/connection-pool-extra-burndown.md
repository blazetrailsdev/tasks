---
title: "Burn down 68 extra (TS-only) tests in connection-pool.test.ts"
status: done
updated: 2026-06-25
...
`packages/activerecord/src/connection-pool.test.ts` carries **68 extra (TS-only) tests** -- `it`/`test`
declarations matching no Rails test (`test:compare` snapshot 2026-06-23,
matched=31, rubyTestCount=31). Per RFC 0043 these are
...
- **One file per PR:** touch only `packages/activerecord/src/connection-pool.test.ts` (optionally its sibling
