---
title: "Burn down 151 extra (TS-only) tests in base.test.ts"
status: done
updated: 2026-06-25
...
`packages/activerecord/src/base.test.ts` carries **151 extra (TS-only) tests** -- `it`/`test`
declarations matching no Rails test (`test:compare` snapshot 2026-06-23,
matched=170, rubyTestCount=170). Per RFC 0043 these are
...
- **One file per PR:** touch only `packages/activerecord/src/base.test.ts` (optionally its sibling
...
- No test renamed or reworded; the remaining file passes
  (`pnpm vitest run packages/activerecord/src/base.test.ts`).
