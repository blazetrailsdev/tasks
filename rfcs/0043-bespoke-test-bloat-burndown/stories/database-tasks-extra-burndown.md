---
title: "Burn down 72 extra (TS-only) tests in database-tasks.test.ts"
status: done
updated: 2026-06-25
...
priority: 16
pr: 4151
claim: "2026-06-25T20:47:15Z"
assignee: "database-tasks-extra-burndown"
blocked-by: null
---

## Context

`packages/activerecord/src/tasks/database-tasks.test.ts` carries **72 extra (TS-only) tests** -- `it`/`test`
declarations matching no Rails test (`test:compare` snapshot 2026-06-23,
matched=78, rubyTestCount=78). Per RFC 0043 these are
...
- **One file per PR:** touch only `packages/activerecord/src/tasks/database-tasks.test.ts` (optionally its sibling
