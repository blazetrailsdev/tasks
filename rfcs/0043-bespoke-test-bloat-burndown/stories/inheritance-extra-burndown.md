...
`packages/activerecord/src/inheritance.test.ts` carries **50 extra (TS-only) tests** -- `it`/`test`
declarations matching no Rails test (`test:compare` snapshot 2026-06-23,
matched=73, rubyTestCount=73). Per RFC 0043 these are
...
- **One file per PR:** touch only `packages/activerecord/src/inheritance.test.ts` (optionally its sibling
