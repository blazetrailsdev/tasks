...
`packages/activerecord/src/prepared-statement-status.test.ts` carries **1 extra (TS-only) tests** -- `it`/`test`
...
`vendor/rails/activerecord/test/cases/prepared_statement_status_test.rb`.

This file's paired Rails test (`prepared_statement_status_test.rb`) maps **zero** tests in `test:compare` (its `it`s are all TS-only). Such connection/adapter infra tests are prime **RELOCATE** candidates (trails-specific invariants -> `*.trails.test.ts`) rather than DELETE -- triage each against the Rails file before removing.
...
- **One file per PR:** touch only `packages/activerecord/src/prepared-statement-status.test.ts` (optionally its sibling
...
- No test renamed or reworded; the remaining file passes
  (`pnpm vitest run packages/activerecord/src/prepared-statement-status.test.ts`).
