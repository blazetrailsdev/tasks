...
priority: 6
pr: null
claim: "2026-06-25T16:14:00Z"
assignee: "where-extra-burndown"
blocked-by: null
---

## Context

`packages/activerecord/src/relation/where.test.ts` carries **71 extra (TS-only) tests** -- `it`/`test`
declarations matching no Rails test (`test:compare` snapshot 2026-06-23,
matched=62, rubyTestCount=62). Per RFC 0043 these are
bloat to retire. Rails counterpart:
`vendor/rails/activerecord/test/cases/relation/where_test.rb`.
...
- **One file per PR:** touch only `packages/activerecord/src/relation/where.test.ts` (optionally its sibling
...
- No test renamed or reworded; the remaining file passes
  (`pnpm vitest run packages/activerecord/src/relation/where.test.ts`).
