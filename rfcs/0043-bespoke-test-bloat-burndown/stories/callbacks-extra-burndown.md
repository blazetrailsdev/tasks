---
title: "Burn down 61 extra (TS-only) tests in callbacks.test.ts"
status: done
updated: 2026-06-25
...
priority: 7
pr: 4133
claim: "2026-06-25T16:30:30Z"
assignee: "callbacks-extra-burndown"
blocked-by: null
---

## Context

`packages/activerecord/src/callbacks.test.ts` carries **61 extra (TS-only) tests** -- `it`/`test`
declarations matching no Rails test (`test:compare` snapshot 2026-06-23,
matched=21, rubyTestCount=21). Per RFC 0043 these are
...
- **One file per PR:** touch only `packages/activerecord/src/callbacks.test.ts` (optionally its sibling
...
- No test renamed or reworded; the remaining file passes
  (`pnpm vitest run packages/activerecord/src/callbacks.test.ts`).
