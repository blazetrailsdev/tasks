---
title: "Burn down 412 extra (TS-only) tests in relations.test.ts"
status: claimed
updated: 2026-06-25
...
priority: 1
pr: null
claim: "2026-06-25T15:17:34Z"
assignee: "relations-extra-burndown"
blocked-by: null
---

## Context

`packages/activerecord/src/relations.test.ts` carries **412 extra (TS-only) tests** -- `it`/`test`
declarations matching no Rails test (`test:compare` snapshot 2026-06-23,
matched=278, rubyTestCount=279). Per RFC 0043 these are
...
- **One file per PR:** touch only `packages/activerecord/src/relations.test.ts` (optionally its sibling
