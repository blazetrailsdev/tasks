...
`packages/activerecord/src/enum.test.ts` carries **107 extra (TS-only) tests** -- `it`/`test`
declarations matching no Rails test (`test:compare` snapshot 2026-06-23,
matched=97, rubyTestCount=97). Per RFC 0043 these are
...
- **One file per PR:** touch only `packages/activerecord/src/enum.test.ts` (optionally its sibling
...
- No test renamed or reworded; the remaining file passes
  (`pnpm vitest run packages/activerecord/src/enum.test.ts`).
