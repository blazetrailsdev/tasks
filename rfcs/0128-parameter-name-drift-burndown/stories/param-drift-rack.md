...
- rack enrols in the gate in this PR: add `"rack"` to `GATED_PACKAGES` in
...
- **Settled in PR #7251 at a mark of 3, not 0.** The 29→3 rows this story
  ...
  their `vendor/rails` `file:line` in [[param-drift-rack-structural-residue]]; the mark is only-shrink and burns
  down there.
