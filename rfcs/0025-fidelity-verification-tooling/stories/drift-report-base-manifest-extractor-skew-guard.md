---
title: "parity:api:drift — guard base/target extractor-version skew"
status: done
updated: 2026-06-24
...
The cross-version drift report (`pnpm parity:api:drift`, scripts/api-compare/drift.ts,
...
`pnpm parity:api` last produced. If the base was built by a DIFFERENT version
...
purely the stale base. The README tells the user to run `parity:api` first,
...
- `parity:api:drift` compares the two; on mismatch either auto-rebuilds the base
