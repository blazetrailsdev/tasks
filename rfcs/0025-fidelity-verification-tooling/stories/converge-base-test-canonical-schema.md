---
title: "converge-base-test-canonical-schema"
status: claimed
updated: 2026-07-02
...
priority: null
pr: null
claim: "2026-07-02T01:29:59Z"
assignee: "converge-base-test-canonical-schema"
blocked-by: null
---
...
`packages/activerecord/src/base.test.ts`, which declares a large bespoke SCHEMA (~60 tables) wrapped in `satisfies Schema` and passes it to
...
To keep that PR scoped to the rule fix, `packages/activerecord/src/base.test.ts` was re-grandfathered in
...
- [ ] Replace the bespoke `satisfies Schema` schema in `packages/activerecord/src/base.test.ts` with the canonical
...
- [ ] Remove `packages/activerecord/src/base.test.ts` from
