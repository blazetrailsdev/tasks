---
title: "converge-attribute-methods-test-canonical-schema"
status: in-progress
updated: 2026-07-02
...
`packages/activerecord/src/attribute-methods.test.ts`, which declares topics (no last_read), posts, plus items/people etc. wrapped in `as const` and passes it to
...
To keep that PR scoped to the rule fix, `packages/activerecord/src/attribute-methods.test.ts` was re-grandfathered in
...
- [ ] Replace the bespoke `as const` schema in `packages/activerecord/src/attribute-methods.test.ts` with the canonical
...
- [ ] Remove `packages/activerecord/src/attribute-methods.test.ts` from
