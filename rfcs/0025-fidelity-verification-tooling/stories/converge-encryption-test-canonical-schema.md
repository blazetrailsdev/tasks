---
title: "converge-encryption-test-canonical-schema"
status: in-progress
updated: 2026-07-02
...
`packages/activerecord/src/encryption.test.ts`, which declares a bespoke TEST_SCHEMA wrapped in `as const` and passes it to
...
To keep that PR scoped to the rule fix, `packages/activerecord/src/encryption.test.ts` was re-grandfathered in
...
- [ ] Replace the bespoke `as const` schema in `packages/activerecord/src/encryption.test.ts` with the canonical
...
- [ ] Remove `packages/activerecord/src/encryption.test.ts` from
