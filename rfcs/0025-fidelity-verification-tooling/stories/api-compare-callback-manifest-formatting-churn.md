---
title: "parity:api rewrites tracked rails-callback-invocations.json with formatting churn"
status: closed
updated: 2026-07-20
...
Running `pnpm parity:api` rewrites the tracked file
...
covers), so every agent who runs `parity:api` dirties their tree and risks
...
- `pnpm parity:api` on a clean tree leaves `eslint/rails-callback-invocations.json`
