---
title: "parity:api regenerates eslint/rails-*.json unformatted, dirtying every worktree"
status: closed
updated: 2026-07-20
...
Surfaced by PR #4865. `pnpm parity:api` regenerates
...
```console
$ API_COMPARE_FORCE=1 pnpm parity:api && git status --porcelain
...
parity:api: the file must be manually `git checkout`-ed before committing, or
...
`Wrote .../eslint/rails-*.json` lines at the end of an parity:api run.

## Acceptance criteria

- `pnpm parity:api` on a clean tree leaves `git status` clean (no formatting-only
...
- `npx prettier --check eslint/` passes immediately after an parity:api run.
