...
someone runs `pnpm parity:api` in that worktree.
...
then after `pnpm parity:api` populated the manifest (604 files, 4975 names)
...
Related but distinct from `parity:api does not run the wide ratchet lint`.
...
- [ ] A missing `rails-api.json` fails loudly (non-zero exit with a message
      naming `pnpm parity:api`) instead of writing an empty manifest and
