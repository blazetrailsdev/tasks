...
priority: 0
pr: 7638
claim: "2026-09-09T10:30:34Z"
assignee: "stats-sync-20260909"
blocked-by: null
closed-reason: null
---
...
The nightly trails stats sync (host crontab → `scripts/sync-stats/cron-wrapper.sh`, daily at 06:00 America/New_York) failed on its 2026-09-09 run. Until it is fixed, `stats.db` stops receiving new PRs, workflow runs, job logs, and api/test:compare rows, so every dashboard and query built on it silently goes stale.

The wrapper appends each run to `/home/dean/github/blazetrailsdev/stats-sync.log`; this is the failing output:

```text
[ELIFECYCLE] Command failed with exit code 1.
[cron-wrapper] stats sync failed at stage "startup" (exit 1): Node.js v24.16.0
```
