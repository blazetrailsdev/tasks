---
title: "Port has-one-associations.test.ts to canonical schema"
status: claimed
updated: 2026-06-16
...
priority: 1
pr: null
claim: "2026-06-16T14:34:37Z"
assignee: "assoc-has-one"
blocked-by: null
---

## Context

Convert `packages/activerecord/src/associations/has-one-associations.test.ts` (~1557 LOC, **38 inline tables**)
...
Rails drives `Firm`/`Account`/`Author`/`Ship`/`Pirate` has_one with dependent + build/replace — all canonical. The inline tables here are per-test
...
- [ ] **Converged setup, not `defineSchema`:** wire the file with
      ...
      `createTestAdapter`.
- [ ] Open `associations/has_one_associations_test.rb` FIRST; port each body word-for-word — same assertions,
...
- [ ] Replace bespoke `class X extends Base` with the canonical registry models
      (`Firm`/`Account`/`Author`/`Ship`/`Pirate` has_one with dependent + build/replace) and their Rails associations; rows via `fixtures` +
...
- [ ] `pnpm vitest run packages/activerecord/src/associations/has-one-associations.test.ts` passes (co-run colliding
      siblings under `maxForks=1`).

## Notes

- ~1557 LOC, 38 tables: this is multi-PR. Split per-describe across sibling
