...
Convert `packages/activerecord/src/relation/predicate-builder.test.ts` (~350
LOC, 7 inline tables) onto the canonical schema, matched to Rails.

- trails: `relation/predicate-builder.test.ts`
- Rails: `vendor/rails/activerecord/test/cases/relation/predicate_builder_test.rb`

Rails drives `Topic`/`Post`/`Author` for predicate-builder coercion and
association-key resolution — all canonical. Ride the canonical tables.
...
- [ ] **Converged setup, not `defineSchema`:** wire the file with
      ...
      `createTestAdapter`.
- [ ] Open `predicate_builder_test.rb` FIRST; port each body word-for-word.
...
- [ ] `pnpm vitest run packages/activerecord/src/relation/predicate-builder.test.ts`
      passes.
