...
Convert `packages/activerecord/src/associations/has-many-through-associations.test.ts` (~8321 LOC, **488 inline tables**)
...
Rails drives `Author`/`Post`/`Tag`/`Tagging`/`Category`/`Reader` has_many :through chains — all canonical. The inline tables here are per-test
...
- [ ] **Converged setup, not `defineSchema`:** wire the file with
      ...
      `createTestAdapter`.
- [ ] Open `associations/has_many_through_associations_test.rb` FIRST; port each body word-for-word — same assertions,
...
- [ ] Replace bespoke `class X extends Base` with the canonical registry models
      (`Author`/`Post`/`Tag`/`Tagging`/`Category`/`Reader` has_many :through chains) and their Rails associations; rows via `fixtures` +
...
- [ ] `pnpm vitest run packages/activerecord/src/associations/has-many-through-associations.test.ts` passes (co-run colliding
      siblings under `maxForks=1`).

## Notes

- ~8321 LOC, 488 tables: this is multi-PR. Split per-describe across sibling
