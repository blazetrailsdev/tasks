...
Convert `packages/activerecord/src/associations.test.ts` (~10196 LOC, **348 inline tables**)
...
Rails drives `Developer`/`Project`/`Firm`/`Account`/`Author`/`Post` association-proxy + reflection basics — all canonical. The inline tables here are per-test
...
- [ ] **Converged setup, not `defineSchema`:** wire the file with
      ...
      `createTestAdapter`.
- [ ] Open `associations_test.rb` FIRST; port each body word-for-word — same assertions,
...
- [ ] Replace bespoke `class X extends Base` with the canonical registry models
      (`Developer`/`Project`/`Firm`/`Account`/`Author`/`Post` association-proxy + reflection basics) and their Rails associations; rows via `fixtures` +
...
- [ ] `pnpm vitest run packages/activerecord/src/associations.test.ts` passes (co-run colliding
      siblings under `maxForks=1`).

## Notes

- ~10196 LOC, 348 tables: this is multi-PR. Split per-describe across sibling
