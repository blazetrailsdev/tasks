...
One link in RFC 0135's flip chain: repoint `packages/actionpack/src and packages/actionview/src` from
...
Measured 2026-09-02: **14 files, 49 member calls (actionpack 12/47, actionview 2/2)**.
...
- No `getFs()` or `getPath()` call remains in `packages/actionpack/src and packages/actionview/src`; a grep for
