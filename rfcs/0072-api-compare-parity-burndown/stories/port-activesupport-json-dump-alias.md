...
Attribution note: `json.rb` itself is only two `require`s, so api:compare
...
- [ ] `ActiveSupportJSON.dump` exists, aliasing `encode` with an identical
      ...
      reimplementation.
- [ ] Verify against `pnpm api:compare` which TS file the comparison expects
...
- [ ] `pnpm api:extra --package activesupport` reports no new novel names.
