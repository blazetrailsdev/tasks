...
up front. The btwhooks `trails.pr_merged` and `trails.cycles_exhausted` messages
...
1. **Periodic triage cadence.** Surfaced stories accumulate as `draft`. A recurring
...
- 2026-06-11: initial RFC — standing fallback bucket for port-surfaced deviations and
  follow-ups; created alongside the btwhooks messages.json change that routes findings
  to `pnpm tasks new`.
