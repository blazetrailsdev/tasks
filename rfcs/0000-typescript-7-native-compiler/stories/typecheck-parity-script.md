...
- New `scripts/typecheck-parity.mjs` runs both `tsc --build` and
  `tsgo --build` over the same graph and diffs diagnostics normalized by
  file/line/code, whitespace-insensitive on message text.
...
- The script also diffs emitted `.d.ts` for a sampled set of public entry
  points (tsgo's declaration emit "differs intentionally").
