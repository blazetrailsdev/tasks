...
priority: null
pr: 6873
claim: "2026-08-22T19:04:58Z"
assignee: "wave-5c-head-sweep"
blocked-by: null
closed-reason: null
---
...
The seed-placeholder cluster plus two adapter/finder shards (40 rows):

    activerecord/connection-adapters/postgresql-adapter.json   9
    activerecord/relation/finder-methods.json                  9
    activerecord/store.json                                    8  (all 8 seed)
    activerecord/enum.json                                     7  (all 7 seed)
    activerecord/reflection.json                               7  (6 seed)

`store`, `enum` and `reflection` are the densest seed-placeholder shards in the
repo: each row needs convergence or a real per-site reason read against the
Rails body before it can leave.
