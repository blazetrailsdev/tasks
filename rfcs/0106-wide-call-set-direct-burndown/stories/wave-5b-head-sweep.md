...
The three largest remaining head shards (44 rows):

    activerecord/schema-dumper.json            17
    activerecord/relation/query-methods.json   14
    activerecord/migration.json                13

`schema-dumper.json` and `query-methods.json` were measured clean of seed
placeholders on 2026-08-21 and migrate as-is; `migration.json` holds 2 seed
rows that need real disposition first.
